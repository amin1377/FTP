...
class ConfigFileContainer(object):#TODO:It is not complete
	...
	def __init__(self, json_str, user_name_email, password_email):
		...
		ConfigFileContainer.user_name_email = user_name_email
		ConfigFileContainer.password_email = password_email 
	...
	def process_accounting(cls, user_name, data_size):
		# print("inside process_accounting")
		if(cls.accounting_data["enable"] == False):
			return True
		else:
			account = ""

			##find user name account in users dictionary
			...
			if not account:
				# print("Correspond account has not found")
				return True
			else:
				# print("Check accound size")
				if(account["size"] > data_size):
					# print("account is greater than data_size")
					account["size"] -= data_size
					if account["size"] <= cls.accounting_data["threshold"] and account["alert"] == True:
						cls.send_email(user_name, account)
					return True
				else:
					# print("account is less than data_size")
					cls.send_email(user_name, account)
					return False

	@classmethod
	def send_email(cls, username, account):
		current_size = account['size']
		# print("inside send email")
		username = cls.user_name_email
		password = cls.password_email
...
class ClientRecord(object):
	...
	def decode_mkd_cmd(self):
		if(len(self.cmd) > 2):
			if(self.cmd[1] == "-i"):
				...
				self.write_log(f'couldn\'t create file:  {self.cmd[2]}')
		else:
		...
		if(len(self.cmd) > 2):
			if(self.cmd[1] == "-f"):
				if(os.path.isdir(f'{self.cmd[2]}')):
					...
					self.send_by_cmd_channel("501 Syntax error in parameters or arguments.")
					self.write_log(f'couldn\'t delete dir : {self.cmd[2]}')
			else:
				self.send_by_cmd_channel("501 Syntax error in parameters or arguments.")
				self.write_log(f'couldn\'t delete dir : {self.cmd[2]}')
		else:
			if(os.path.isfile(self.cmd[1])):
				if(ConfigFileContainer.auth["enable"] == False):
					...
					if(ConfigFileContainer.file_auth_check(self.cmd[1])):
						if(ConfigFileContainer.admin_check(self.user_name)):
							...
							self.write_log(f'couldn\'t access file : {self.cmd[1]}')
					else:
						os.remove(self.cmd[1])
						self.send_by_cmd_channel(f'250 {self.cmd[1]} deleted.')
						self.write_log(f'file : {self.cmd[1]} deleted')
				...
				self.send_by_cmd_channel("501 Syntax error in parameters or arguments.")
				self.write_log(f'couldn\'t delete file : {self.cmd[1]}')
	...
	def decode_cwd_cmd(self):
		if(len(self.cmd) == 1):
			...
			if(os.path.isdir(self.cmd[1])):
				...
				self.send_by_cmd_channel("501 Syntax error in parameters or arguments.")
				self.write_log(f'couldn\'t change dir to {self.cwd}')
		self.cwd = os.getcwd()
	...
	def send_dl_data(self):
		...
		if(transmit_is_finish == True):
			...
			self.send_by_cmd_channel("500 Problem with accounting.")
	...
	def decode_dl_cmd(self):#TODO:data_channel
		if(os.path.isfile(self.cmd[1])):
			...
			self.send_by_cmd_channel("501 Syntax error in parameters or arguments.")
			self.write_log(f'could\'t download the file: {self.cmd[1]}')
...
if __name__ == "__main__":
	...
	server_email_addr = input("Enter Server Email Address : ")
	server_email_pass = input("Enter Server Email Address : ")

	config_container = ConfigFileContainer(config_data, server_email_addr, server_email_pass)
